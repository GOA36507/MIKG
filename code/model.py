...
class MIKG:
    ...
    def generate_variables(self):
        ...
        if self.input_value is not None:
            ...
            # no_attr 会用于构建 value_embeddings
    ...
    def build_training_graph(self):
        ...
        input_embeds_att = tf.nn.embedding_lookup(embed_list[0], self.input_entities1)
        input_embeds_rel = tf.nn.embedding_lookup(embed_list[-1], self.input_entities1)

        self.loss +=  self.nce_estimator.forward(input_embeds_att, embed_list[0], self.mi_label) 
        self.loss +=  self.nce_estimator.forward(input_embeds_rel, embed_list[-1], self.mi_label) 

        # Basic CE Loss
        all_ent_embeddings = tf.concat(embed_list, axis=1)
